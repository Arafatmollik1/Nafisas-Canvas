          ...
          {productsData.map((product) => (
            <div key={product.id} className="lg:w-1/3 md:w-1/2 p-4 w-full">
              <div className="border border-gray-200 rounded-lg overflow-hidden">
                <div className="bg-gray-100 p-2"> {/* Increase padding here */}
                  <img 
                    alt={product.name}
                    className="object-contain h-48 w-full"  // Use object-contain to ensure image is contained within its element
                    src={product.imageUrl}
                  />
                </div>
                <div className="p-6"> {/* Padding around text content */}
